...
class Page:
    ...
    def LoadDailyFrame(self):
        ...
        UIMaker.TextLabel(self.F('Daily_Menu'), '종가(KRW)', self.Fo('Menu'), 'white', Col_titleR).place(relx=0.20,
                                                                                                       rely=0.5,
                                                                                                       anchor=W)
        UIMaker.TextLabel(self.F('Daily_Menu'), '전일대비', self.Fo('Menu'), 'white', Col_titleR).place(relx=0.55, rely=0.5,
                                                                                                    anchor=W)
        UIMaker.TextLabel(self.F('Daily_Menu'), '거래', self.Fo('Menu'), 'white', Col_titleR).place(relx=0.78, rely=0.5,
                                                                                                  anchor=W)
        for i in range(0, 5):
            P = self.F('Daily_' + str(i + 1))
            Col = Col_title
    ...
    def ChangeFunction(self, tag):
        ...
        if tag is 'Auto':
            ...
            if self.Auto:
                str = tag + '_On'
                self.AutoPage.Add('bitcoin', self.CurrCoin.koreanName)
            else:
                str = tag + '_Off'
                self.AutoPage.Delete('bitcoin', self.CurrCoin.koreanName)
    ...
    def ResetFunction(self):
        ...
        if self.AutoPage.Find('bitcoin', self.CurrCoin.koreanName) is False :
            self.Auto = False
    ...
    def SetGraph(self):
        ...
        if self.anim is not None:
            self.anim._stop()
        self.anim = FuncAnimation(self.fig, self.UpdateGraph, init_func=self.InitGraph, interval=1000, frames=60,
                                  blit=False)
        self.Canvas.draw()
    ...
    def SetAniGraph(self):
        ...
        if self.anim is not None:
            self.anim._stop()
        self.anim = FuncAnimation(self.fig, self.UpdateAniGraph, init_func=self.InitAniGraph, interval=1000, frames=10,
                                  blit=False)
        self.Canvas.draw()
